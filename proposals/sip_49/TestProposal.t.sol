...
contract TestProposal is GovTestHelper {
    ...
    address constant EXPECTED_NEW_BASE_LEVERAGE_TOKEN_IMPLEMENTATION =
        0x603Da735780e6bC7D04f3FB85C26dccCd4Ff0a82;
    address constant EXPECTED_NEW_BASE_LEVERAGE_MANAGER_IMPLEMENTATION =
    ...
    function test_baseLeverageTokenImplementationIsUpgraded_afterPassingProposal(
    ) public {
    ...
    function test_baseLeverageManagerImplementationIsUpgraded_afterPassingProposal(
    ) public {
    ...
    function test_bothImplementationsAreUpgraded_afterPassingProposal()
        public
    {
        ...
        assertNotEq(
            tokenImplementationBefore,
            EXPECTED_NEW_BASE_LEVERAGE_TOKEN_IMPLEMENTATION
        );
        assertNotEq(
            managerImplementationBefore,
            EXPECTED_NEW_BASE_LEVERAGE_MANAGER_IMPLEMENTATION
        );
    ...
    function test_proposalActionsMatchExpected() public view {
        ...
        assertEq(
            proposal.getTargets().length,
            3,
            "Proposal should have exactly 3 actions"
        );
    ...
    function test_accessControlIsHandledCorrectly() public {
        ...
        bool hasRoleBefore = IAccessControl(
            SeamlessAddressBook.BASE_LEVERAGE_MANAGER_PROXY
        ).hasRole(upgraderRole, SeamlessAddressBook.TIMELOCK_SHORT);
        assertFalse(
            hasRoleBefore,
            "Timelock should not have UPGRADER_ROLE before proposal"
        );
        ...
        bool hasRoleAfter = IAccessControl(
            SeamlessAddressBook.BASE_LEVERAGE_MANAGER_PROXY
        ).hasRole(upgraderRole, SeamlessAddressBook.TIMELOCK_SHORT);
        assertTrue(
            hasRoleAfter,
    ...
    function test_specificLeverageTokenImplementationIsUpgraded() public {
        ...
        address beacon =
            address(uint160(uint256(vm.load(leverageToken, beaconSlot))));
