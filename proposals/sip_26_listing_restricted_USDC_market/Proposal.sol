  ...
  function _makeProposal() internal virtual override {
    ...
    _addAction(
      ...
      abi.encodeWithSelector(
        IPoolConfigurator.initReserves.selector,
        initReserveInputs
      )
    );
    ...
    sources[0] = IAaveOracle(SeamlessAddressBook.AAVE_ORACLE).getSourceOfAsset(SeamlessAddressBook.USDC);

    _addAction(
      SeamlessAddressBook.AAVE_ORACLE,
      abi.encodeWithSelector(
        IAaveOracle.setAssetSources.selector,
        assets,
        sources
      )
    );
